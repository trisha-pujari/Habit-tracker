...
import { lastNDays, calculateStreak, todayKey } from "../utils/dateHelpers.js";
...
export const weeklyReport = async (req, res) => {
  try {
    ...
    const userMsg = `Here is the user's habit data for the past 7 days (${ctx.days[0]} to ${ctx.days[ctx.days.length - 1]}):\n\n${ctx.perHabit
      .map(
        (h) =>
...
export const suggestHabits = async (req, res) => {
  try {
    ...
    try {
      ...
      console.error("Failed to parse AI habit suggestions:", parseErr.message, "\nRaw content:", content);
      suggestions = [];
    }
...
export const recoveryPlan = async (req, res) => {
  try {
    ...
    const logs = await HabitLog.find({
      userId: req.user._id,
      habitId,
    }).sort({ completedDate: 1 }); // FIX: ascending order so calculateStreak receives dates oldest→newest

    const keys = logs.map((l) => l.completedDate);
    const { current, longest } = calculateStreak(keys);

    const userMsg = `Habit: ${habit.name} (${habit.category}).\nDescription: ${habit.description || "none"}.\nCurrent streak: ${current} days. Longest streak ever: ${longest} days. The user just broke a streak. Write a warm, actionable 3-day recovery plan.`;
    ...
    const context = habits
      .map((h) => {
        ...
        for (const l of hLogs) {
          // FIX: parse date string as local time (append T00:00:00) to avoid UTC offset shifting the day
          const dow = new Date(l.completedDate + "T00:00:00").getDay();
          byDow[dow] += 1;
        }
    ...
    const ctx = habits
      .map((h) => {
        const hLogs = logs
          ...
          .sort(); // FIX: ascending sort to match calculateStreak's expected order
        const { current } = calculateStreak(hLogs);
        return `${h.name}: current streak ${current}`;
      })
      .join("\n");

    const today = todayKey(); // e.g. "2025-05-20"
    // FIX: normalise both sides to a plain date string to avoid type mismatch
    const todayLogs = logs.filter(
      (l) => String(l.completedDate).slice(0, 10) === today
    );
    const done = todayLogs.length;
    const total = habits.length;

    const userMsg = `Today's habits and streaks:\n${ctx}\n\nDone today: ${done}/${total}. Write the morning motivation message.`;
